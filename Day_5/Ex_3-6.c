...
/*				        3. min_fwidth -- minimum field width to add as padding with the converted value	 			        */
/*        																						     																											*/
/* RETURN 	   :  nothing																		     			                                          */
/* NOTES 	   :  integer to character conversion function handling the smallest negetive number with padding	    */
/****************************************************************************************************************/
void i_to_a(int n, char s[], int min_fwidth) {
    int i, sign;
    sign = n;
    i = 0;
    do { 
...
int main(void) {
  	...
  	scanf("%d", &min_field_width);
  	printf("Enter the integer value to convert: ");
  	scanf("%d", &n);
  	char s[MAXLEN];
  	i_to_a(n, s, min_field_width);
  	printf("The Converted String from Integer is:\n%s\n", s);
